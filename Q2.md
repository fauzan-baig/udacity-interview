...
Instructor service will immediately publish **OfficeHourEvent** to notify the students assigned to their course. If the service has active appointments (**CreateNewOfficeHourAppointmentEvent**), those appointments available to the instructor for the instructor to accepts or ignore. If the instructor choose to accept the appointment the service publishes **OfficeHourAppointmentAcceptedEvent** acknowledging the appointment to the student. Instructor will join the meeting and wait for 5min till the student shows up. Items are removed from the queue immeditely after the appointment is accepted. Instructor service buffers **CreateNewOfficeHourAppointmentEvent**, allowing the service to maintain appointment events (about the students currently waiting in the office lobby for doubts or discussions). 

Students that are notified will have to publish **CreateNewOfficeHourAppointmentEvent** to meet the instructor. This request is on FIFO basis and instructor may choose the next student in line by accepting the event which then publishes **OfficeHourAppointmentAcceptedEvent**. This is done to ensure that students don't rush during the office hours. Instructors don't necessarily choose the next student, it is done automatically by the message queue as the queue is in FIFO order. Items are removed from the queue immeditely after the appointment is accepted. 

![1b](resources/1c.jpg)

### Additional Improvements
We are assuming the following, student that has published an appointment first will be given the prirotity even though they may have iniaited the request many hours ago. It is the students responsibility to join the meeting when an instructor makes an invite. We can further improve the system by adding a batching and prioritization service that takes active students pushes them on top of the queue and immediately reject the students that are waiting for more than an hour. 
